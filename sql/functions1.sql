...
begin
  ...
  else if (new.d10 <> (select d10 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':10:' || new.d10::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 12 for 1) <> '1') then new.d11 = old.d11;
  else if (new.d11 <> (select d11 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':11:' || new.d11::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 13 for 1) <> '1') then new.d12 = old.d12;
  else if (new.d12 <> (select d12 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':12:' || new.d12::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 14 for 1) <> '1') then new.d13 = old.d13;
  else if (new.d13 <> (select d13 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':13:' || new.d13::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 15 for 1) <> '1') then new.d14 = old.d14;
  else if (new.d14 <> (select d14 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':14:' || new.d14::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 16 for 1) <> '1') then new.d15 = old.d15;
  else if (new.d15 <> (select d15 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':15:' || new.d15::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 17 for 1) <> '1') then new.d16 = old.d16;
  else if (new.d16 <> (select d16 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':16:' || new.d16::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 18 for 1) <> '1') then new.d17 = old.d17;
  else if (new.d17 <> (select d17 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':17:' || new.d17::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 19 for 1) <> '1') then new.d18 = old.d18;
  else if (new.d18 <> (select d18 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':18:' || new.d18::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 20 for 1) <> '1') then new.d19 = old.d19;
  else if (new.d19 <> (select d19 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':19:' || new.d19::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 21 for 1) <> '1') then new.d20 = old.d20;
  else if (new.d20 <> (select d20 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':20:' || new.d20::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 22 for 1) <> '1') then new.d21 = old.d21;
  else if (new.d21 <> (select d21 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':21:' || new.d21::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 23 for 1) <> '1') then new.d22 = old.d22;
  else if (new.d22 <> (select d22 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':22:' || new.d22::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 24 for 1) <> '1') then new.d23 = old.d23;
  else if (new.d23 <> (select d23 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':23:' || new.d23::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 25 for 1) <> '1') then new.d24 = old.d24;
  else if (new.d24 <> (select d24 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':24:' || new.d24::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 26 for 1) <> '1') then new.d25 = old.d25;
  else if (new.d25 <> (select d25 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':25:' || new.d25::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 27 for 1) <> '1') then new.d26 = old.d26;
  else if (new.d26 <> (select d26 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':26:' || new.d26::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 28 for 1) <> '1') then new.d27 = old.d27;
  else if (new.d27 <> (select d27 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':27:' || new.d27::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 29 for 1) <> '1') then new.d28 = old.d28;
  else if (new.d28 <> (select d28 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':28:' || new.d28::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 30 for 1) <> '1') then new.d29 = old.d29;
  else if (new.d39 <> (select d29 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':29:' || new.d29::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 31 for 1) <> '1') then new.d30 = old.d30;
  else if (new.d30 <> (select d30 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':30:' || new.d30::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 32 for 1) <> '1') then new.d31 = old.d31;
  else if (new.d31 <> (select d31 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':31:' || new.d31::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 33 for 1) <> '1') then new.d32 = old.d32;
  else if (new.d32 <> (select d32 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':32:' || new.d32::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 34 for 1) <> '1') then new.d33 = old.d33;
  else if (new.d33 <> (select d33 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':33:' || new.d33::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 35 for 1) <> '1') then new.d34 = old.d34;
  else if (new.d34 <> (select d34 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':34:' || new.d34::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 36 for 1) <> '1') then new.d35 = old.d35;
  else if (new.d35 <> (select d35 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':35:' || new.d35::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 37 for 1) <> '1') then new.d36 = old.d36;
  else if (new.d36 <> (select d36 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':36:' || new.d36::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 38 for 1) <> '1') then new.d37 = old.d37;
  else if (new.d37 <> (select d37 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':37:' || new.d37::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 39 for 1) <> '1') then new.d38 = old.d38;
  else if (new.d38 <> (select d38 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':38:' || new.d38::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 40 for 1) <> '1') then new.d39 = old.d39;
  else if (new.d39 <> (select d39 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':39:' || new.d39::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 41 for 1) <> '1') then new.d40 = old.d40;
  else if (new.d40 <> (select d40 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':40:' || new.d40::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 42 for 1) <> '1') then new.d41 = old.d41;
  else if (new.d41 <> (select d41 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':41:' || new.d41::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 43 for 1) <> '1') then new.d42 = old.d42;
  else if (new.d41 <> (select d42 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':42:' || new.d42::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 44 for 1) <> '1') then new.d43 = old.d43;
  else if (new.d43 <> (select d43 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':43:' || new.d43::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 45 for 1) <> '1') then new.d44 = old.d44;
  else if (new.d44 <> (select d44 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':44:' || new.d44::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 46 for 1) <> '1') then new.d45 = old.d45;
  else if (new.d45 <> (select d45 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':45:' || new.d45::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 47 for 1) <> '1') then new.d46 = old.d46;
  else if (new.d46 <> (select d46 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':46:' || new.d46::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 48 for 1) <> '1') then new.d47 = old.d47;
  else if (new.d47 <> (select d47 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':47:' || new.d47::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 49 for 1) <> '1') then new.d48 = old.d48;
  else if (new.d48 <> (select d48 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':48:' || new.d48::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 50 for 1) <> '1') then new.d49 = old.d49;
  else if (new.d49 <> (select d49 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':49:' || new.d49::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 51 for 1) <> '1') then new.d50 = old.d50;
  else if (new.d50 <> (select d50 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':50:' || new.d50::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 52 for 1) <> '1') then new.d51 = old.d51;
  else if (new.d51 <> (select d51 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':51:' || new.d51::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 53 for 1) <> '1') then new.d52 = old.d52;
  else if (new.d52 <> (select d52 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':52:' || new.d52::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  if (substring(mode from 54 for 1) <> '1') then new.d53 = old.d53;
  else if (new.d53 <> (select d53 from status_dio where unit = new.unit)) then new.updated = now(); extra := new.unit::text || ':53:' || new.d53::text; perform pg_notify('req_dio_arduino_mqtt', extra); end if;
  end if;
  --raise notice '%', extra;
  return new;
end;
...
begin
  ...
  new.updated = now();
  update unit set updated = now() where unit = new.unit;
  return new;
end;
...
begin
  ...
  if ((new.i00 <> old.i00) or (new.i01 <> old.i01) or (new.i02 <> old.i02)) then
    extra := new.unit::text || ':' || new.i00::text || '|' || new.i01::text || '|' || new.i02::text || '|';
